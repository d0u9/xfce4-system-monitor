...
static GtkWidget *create_drop_down(GtkBox *tab, GtkSizeGroup *sg,
				   ...
				   gsize num_items, guint init);
static GtkBox *create_option_line(GtkBox *tab, GtkSizeGroup *sg,
				  const gchar *name);
...
void menu_properties(XfcePanelPlugin *plugin, sys_monitor_t *base)
{
	...
	gw_conf = xfce_titled_dialog_new_with_buttons
		("Configuration",
	...
	/* close */
	g_signal_connect(gw_conf, "response", G_CALLBACK(cb_response), base);

	gtk_window_set_icon_name(GTK_WINDOW(gw_conf), ICON);
	...
	gtk_notebook_append_page(GTK_NOTEBOOK(gw_notebook),
				 GTK_WIDGET(gb_tab1), GTK_WIDGET(label));

	//label = gtk_label_new("Others");
	//gtk_notebook_append_page(GTK_NOTEBOOK(gw_notebook),
				 //GTK_WIDGET(gb_tab2), GTK_WIDGET(label));

	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(gw_conf)->vbox),
			   GTK_WIDGET(gw_notebook), TRUE, TRUE, 0);

	gtk_widget_show(gw_conf);
}
